...
enum class PacketType : uint8_t {
    ...
    TELEMETRY    = 0x04,   // Rover -> Ground : esp_now_data_t standard drone telemetry
    COMMAND      = 0x05,   // Ground -> Rover : esp_now_cmd_t (e.g. ESTOP)
};
...
// Called when a command data packet (e.g. E-STOP) arrives.
using CommandCallback = void (*)(const uint8_t* data, size_t len, const uint8_t* src_mac);

// Called when an ESP-NOW TX completes.
using SendCallback = void (*)(const uint8_t* mac_addr, esp_now_send_status_t status);
esp_err_t wifi_init();
...
// Register the callback for arbitrary commands mapped to COMMAND type packets.
void set_command_callback(CommandCallback cb);

// Register the callback for TX completion tracking.
